...
################################################################################
#### Função principal para dividir o arquivo
...
def split_pdf_by_employees():
    try:
        ...
        for i in range(total_steps):
            ...
            div_mercadoria_pdf.update_idletasks()  # Atualiza a interface
...
def exibir_mensagem_erro(mensagem):
    janela_erro = ctk.CTkToplevel(div_mercadoria_pdf)
    ...
    janela_erro.resizable(width=False, height=False)
    ...
    div_mercadoria_pdf.wait_window(janela_erro)
    
################################################################################
#### Função para exibir mensagem de conclusão:

def conclusao():
    jan_conclusao = ctk.CTkToplevel(div_mercadoria_pdf)
    ...
    div_mercadoria_pdf.wait_window(jan_conclusao)
...
botao_dividir = ctk.CTkButton(master=div_mercadoria_pdf, text='Dividir pdf', command=iniciar_processo)
...
barra_progresso = ctk.CTkProgressBar(master=div_mercadoria_pdf, width=900)
barra_progresso.grid(row=5, column=0, padx=20, pady=20, columnspan=5)
barra_progresso.set(0)
percentual_processo = ctk.CTkLabel(div_mercadoria_pdf, text="0%")
percentual_processo.grid(row=6, column=0, padx=20, pady=20, columnspan=5)
